// AC5 recorded deliberate duplication (item 135).
//
// The full product `src/enrich/granola-signals.ts` (Granola signal WORKER +
// extraction) is item-133-owned and is NOT copied into echo-context. This file
// is a minimal GENERIC-retrieval duplicate holding only the symbols the
// context tool `search_memories` needs to filter derived Granola signal atoms
// to the current manifest run: the two source-name constants and
// `filterToCurrentSignalRuns` (with its `resolveCurrentGranolaSignalRuns` +
// `parseManifest` helpers). No decision/rationale/action extraction, no worker,
// no checkpoints. Byte-derived from the pinned source's generic subset;
// recorded as deliberate duplication in provenance/parity-matrix.v1.json.
import { GRANOLA_DERIVED_SOURCE, GRANOLA_DERIVED_INDEX_SOURCE } from '../capture/granola-source-policy.js';
